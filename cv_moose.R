...
m2 <- c(1, 199, 0, 200, 230, 1)
rclmat2 <- matrix(m2, ncol = 3, byrow = TRUE)
forest <- reclassify(r2, rclmat2, include.lowest=TRUE, right = NA)
...
# Fit the RSF ###
rsf <- ctmm:::rsf.fit(train, UD=ud, R=list(wl=wl,forest=forest), debias=TRUE, error=0.01)
summary(rsf)
print("Fitted RSF")

# Calculate the RSF-informed AKDE
ud_rsf <- akde(train, rsf, R=list(wl=wl,forest=forest))
...
write.table(results, 'results/cv_summary_moose.csv', append=TRUE, row.names=FALSE, col.names=FALSE, sep=',') 
