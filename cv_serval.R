...
m1 <- c(1, 1, 1, 2, 35, 0, 36, 38, 1, 39, 48, 0)
rclmat1 <- matrix(m1, ncol = 3, byrow = TRUE)
water <- reclassify(r2, rclmat1, include.lowest=TRUE, right = NA)
...
bu <- reclassify(r2, rclmat2, include.lowest=TRUE, right = NA)
bu@data@values <- as.logical(bu@data@values)
...
# Fit the RSF ###
rsf <- ctmm:::rsf.fit(train, UD=ud, R=list(built-up=bu,water=water), debias=TRUE, error=0.01)
summary(rsf)
print("Fitted RSF")
